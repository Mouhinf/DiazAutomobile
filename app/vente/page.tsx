...
const VentePage = () => {
  const { data: allSaleCars, isLoading, isError } = useQuery<Car[], Error>({
    queryKey: ['allSaleCars'],
  ...
  useEffect(() => {
    if (allSaleCars) {
      let cars = [...allSaleCars]; // Travailler sur une copie
      ...
      if (filters.minPrice) {
        cars = cars.filter(car => {
          const priceValue = parseFloat((car.salePrice || '0').replace(/[^0-9,-]+/g, "").replace(',', '.'));
